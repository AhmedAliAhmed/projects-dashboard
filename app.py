...
# التنسيق العربي المميز لهوية شركة العمران المتقدم
st.markdown("""
<style>
...
# -----------------------------
# 2. البيانات المحدثة طِبقاً لجدول 2026م (شركة العمران المتقدم)
# -----------------------------
initial_data = [
...
with tab1:
    # 1. الجدول التفاعلي (التحرير المباشر ينعكس فورياً)
    st.markdown('<div class="section-title">✏️ جدول تعديل المستخلصات (أي تعديل رقمي في الخلايا ينعكس فورياً على البطاقات والمخططات)</div>', unsafe_allow_html=True)
    ...
    # حفظ حالة التعديلات في الجلسة تلقائياً
    st.session_state.df_projects = edited_df

    # 2. حساب الإجماليات والنسب فورياً من البيانات المعدلة
    ...
    # 6. تصدير التقرير الرسمي وإكسل
    st.markdown("---")
    st.markdown("### 📑 تصدير البيانات والتقرير الرسمي (محدث مع التعديلات)")
    ...
    with btn_col1:
        st.download_button(
            label="📑 تصدير واستخراج التقرير PDF / HTML",
